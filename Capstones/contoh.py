...
def doMenu(filename):
    ...
    while choice != 0:
        ...
        print("3  -  Edit")
        print("-----------")
        choice = keyboardInput(int, "Choice(0,1,2,3): ",
...
def addProduct(filename):
    try:
        ...
        quantity = keyboardInput(
            int, "Quantity: ", "Quantity must be interger")
    ...
    try:
        ...
        for index, line in enumerate(lines):
            ...
            if (index == 0):
                ...
                print(
                    f"{index:<5}{product:20}{int(quantity):>20}{float(price):>20.2f}")
    except Exception as e:
...
def editProduct(filename):
    try:
        ...
        if (index >= len(data)):  # type: ignore
            ...
            confirm = keyboardInput(
                str, "Are you sure? (y/n): ", "Incorrect response")
            if (confirm == "y"):
                newproduct = keyboardInput(
                    str, f"Product[{product}]: ", "Product must be string")
                newquantity = keyboardInput(
                    int, f"Quantity[{quantity}]: ", "Quantity must be interger")
                newprice = keyboardInput(
                    float, f"Price[{price}]: ", "Price must be float")
